...
import { Loader2, FileText, Trash2, Edit, Eye, AlertCircle } from "lucide-react"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
    ...
    async function loadJobDescriptions() {
      ...
      try {
        if (!authState.isAuthenticated || !authState.user?.email) {
          setJobDescriptions([])
          setError("Please sign in to view your job descriptions")
          setIsLoading(false)
          return
        }

        const jds = await JDService.getJDsByEmail(authState.user.email)
    ...
    try {
      ...
      if (success) {
        ...
        if (authState.user?.email) {
          await JDService.trackUserActivity(authState.user.email, "delete_jd", `Deleted job description with ID: ${id}`)
        }
      } else {
        toast({
          title: "Error",
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-grow flex items-center justify-center bg-slate-50 p-4">
          <div className="max-w-md w-full bg-white rounded-lg shadow-md p-8 text-center">
            ...
            <p className="text-slate-600 mb-6">Please sign in to view your job description history.</p>
