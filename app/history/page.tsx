  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (activeTab === "my-jds") {
          const { success, data, error } = await JDService.getUserJDs(authState.user?.email || "")
          if (success && data) {
            setMyJDs(data)
          } else if (error) {
            toast({
          ...
          const { success, data, error } = await JDService.getUserHistory(authState.user?.email || "")
          if (success && data) {
            setMyHistory(data)
          } else if (error) {
            toast({
    ...
    loadData()
  }, [activeTab, toast, authState.isAuthenticated, authState.user?.email])

  const handleDeleteJD = async (id: string) => {
    try {
      const { success, error } = await JDService.deleteJD(id, authState.user?.email || "")
      if (success) {
        // Remove from state
                      ...
                      {allHistory.map((item) => (
                        <div key={item.id} className="p-4 bg-white border border-slate-200 rounded-lg shadow-sm">
                          <div className="flex items-center">
                            ...
                            <div>
                              <p>
                                <span className="font-medium">{item.full_name || "Anonymous User"}</span>{" "}
